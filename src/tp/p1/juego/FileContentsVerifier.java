...
public class FileContentsVerifier {
	...
	public boolean verifyPrefix(String lineFromFile, String prefix) {
	...
	public boolean verifyAlienShipString(String lineFromFile, Game game, int armour) {
		...
		String[] coords = words[1].split (separator2);
		if ( !verifyCoords(Integer.parseInt(coords[0]) , Integer.parseInt(coords[1]) , game) || !verifyLives(Integer.parseInt(words[2]), armour) || !verifyCycleToNextAlienMove(Integer.parseInt(words[3]), game.getLevel()) || ! verifyDir(Boolean.parseBoolean(words[4]))) {
	...
	public static boolean verifyDir(Boolean dir) {
		return dir != null;
	}
